...
class DatariumDB:
    """DatariumDB driver class.

    A :class:`~datariumdb_driver.DatariumDB` driver is able to create, sign,
    ...
    def __init__(self, *nodes, transport_class=Transport, headers=None):
        """Initialize a :class:`~datariumdb_driver.DatariumDB` driver instance.

        Args:
            *nodes (str): DatariumDB nodes to connect to. Currently, the full
            ...
            transport_class: Optional transport class to use.
                Defaults to :class:`~datariumdb_driver.transport.Transport`.
            headers (dict): Optional headers that will be passed with
                ...
                (e.g. :meth:`DatariumDB().transactions.send()
    ...
    @property
    def transport(self):
        """:class:`~datariumdb_driver.transport.Transport`: Object
        responsible for forwarding requests to a
        :class:`~datariumdb_driver.connection.Connection` instance (node).
        """
        return self._transport

    @property
    def transactions(self):
        """:class:`~datariumdb_driver.driver.TransactionsEndpoint`:
    ...
    @property
    def outputs(self):
        """:class:`~datariumdb_driver.driver.OutputsEndpoint`:
    ...
    @property
    def assets(self):
        """:class:`~datariumdb_driver.driver.AssetsEndpoint`:
    ...
    @property
    def blocks(self):
        """:class:`~datariumdb_driver.driver.BlocksEndpoint`:
    ...
    def api_info(self, headers=None):
        ...
        Returns:
            dict: Details of the HTTP API provided by the DatariumDB
            server.
...
class NamespacedDriver:
    ...
    under the :class:`~datariumdb_driver.driver.DatariumDB` driver.
    """
    PATH = '/'

    def __init__(self, driver):
        """Initializes an instance of
        :class:`~datariumdb_driver.driver.NamespacedDriver` with the given
        driver instance.

        Args:
            driver (DatariumDB): Instance of
                :class:`~datariumdb_driver.driver.DatariumDB`.
        """
        self.driver = driver
...
class TransactionsEndpoint(NamespacedDriver):
    ...
    def prepare(*, operation='CREATE', signers=None,
        ...
        Raises:
            :class:`~.exceptions.datariumdbException`: If ``operation`` is
    ...
    def get(self, *, asset_id, operation=None, headers=None):
        ...
        Note:
            Please note that the id of an asset in DatariumDB is
