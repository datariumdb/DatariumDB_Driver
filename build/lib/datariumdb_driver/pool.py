...
class AbstractPicker(metaclass=ABCMeta):
    ...
    def pick(self, connections):
        """Picks a :class:`~datariumdb_driver.connection.Connection`
        instance from the given list of
        :class:`~datariumdb_driver.connection.Connection` instances.
...
class RoundRobinPicker(AbstractPicker):
    """Object to pick a :class:`~datariumdb_driver.connection.Connection`
    instance from a list of connections.

    Attributes:
        picked (str): List index of
            :class:`~datariumdb_driver.connection.Connection`
            instance that has been picked.

    """
    def __init__(self):
        """Initializes a :class:`~datariumdb_driver.pool.RoundRobinPicker`
...
class Pool:
    ...
    def __init__(self, connections, picker_class=RoundRobinPicker):
        """Initializes a :class:`~datariumdb_driver.pool.Pool` instance.
    ...
    def get_connection(self):
        """Gets a :class:`~datariumdb_driver.connection.Connection`
        instance from the pool.

        Returns:
            A :class:`~datariumdb_driver.connection.Connection` instance.
